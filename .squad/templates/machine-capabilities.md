...
> Enable Watcher to skip issues requiring capabilities the current machine lacks.

## Overview

When running Squad across multiple machines (laptops, DevBoxes, GPU servers, Kubernetes nodes), each machine has different tooling. The capability system lets you declare what each machine can do, and Watcher automatically routes work accordingly.
...
Watcher will log skipped issues:
```
...
1. Watcher loads `machine-capabilities.json` at startup
2. For each open issue, Watcher extracts `needs:*` labels
